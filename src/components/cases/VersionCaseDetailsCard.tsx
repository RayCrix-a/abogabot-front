...
interface VersionCaseDetailsCardProps {
  caseData: LawsuitDetailResponse;
}

const VersionCaseDetailsCard: React.FC<VersionCaseDetailsCardProps> = ({ caseData }) => {
  return (
            ...
            {caseData.attorneyOfRecord ? (
              <div className="bg-dark rounded-lg p-4">
                <div className="mb-2 flex items-center">
                  ...
                  <div>
                    <p className="text-white font-medium">{caseData.attorneyOfRecord.fullName}</p>                    {caseData.attorneyOfRecord.idNumber && (
                ...
                {caseData.plaintiffs.map((plaintiff, index) => (
                  <div key={index} className="bg-dark rounded-lg p-4">
                    <div className="mb-2 flex items-center">
                      ...
                      <div>
                        <p className="text-white font-medium">{plaintiff.fullName}</p>                        {plaintiff.idNumber && (
                          <p className="text-gray-400 text-xs flex items-center">
                            <FiHash className="mr-1" size={12} />
                            RUT: {plaintiff.idNumber}
                          </p>
                        )}
                      </div>
                    </div>
                ...
                {caseData.defendants.map((defendant, index) => (
                  <div key={index} className="bg-dark rounded-lg p-4">
                    <div className="mb-2 flex items-center">
                      ...
                      <div>
                        <p className="text-white font-medium">{defendant.fullName}</p>                        {defendant.idNumber && (
                          <p className="text-gray-400 text-xs flex items-center">
                            <FiHash className="mr-1" size={12} />
                            RUT: {defendant.idNumber}
                          </p>
                        )}
                      </div>
                    </div>
...
export default VersionCaseDetailsCard;
